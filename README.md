...
- `KeyboardMap`：帮助建立小鹤双拼编码与键位之间的映射
- `TypingArea`：承载单字、词组、文章三类核心训练内容
- `StatsBar`：在练习进行中持续反馈速度、准确率、进度和连击
- `SettingsPanel`：集中管理训练模式、显示偏好和音效配置
- `HistoryPanel`：用于训练后的趋势复盘和错字分析
- `AuthPanel`：负责邮箱登录、云同步入口与账号联动体验
